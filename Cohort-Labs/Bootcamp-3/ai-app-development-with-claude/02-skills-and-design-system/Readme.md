...
### `/security-foundation`
Reviews the plan for safety issues before any building starts — like a safety inspector walking through blueprints.
...
### `/frontend-setup`
Builds the empty shell of the application with the design system already baked in.
...
### `/design-system`
The skill you use most. Every time Claude builds a new screen, this skill makes sure it matches everything else.
...
- **The five skills and their pipeline order** — `/engineering-planner` → `/implementation-specs` → `/security-foundation` → `/frontend-setup` → `/design-system` form a stage-gated sequence where each skill's output becomes the next skill's input.
