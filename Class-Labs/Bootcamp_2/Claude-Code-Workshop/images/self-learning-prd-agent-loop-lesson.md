...
- **prd-checklist.md** — The 20-point PRD quality standard that drives everything in this loop. Download it from the workshop materials folder and keep it ready. *(If you followed earlier lessons, you already have this file.)*
...
```
Subject: PRD Pattern Update — Lease Compliance Tool (Cycle 3)

Hi Mahesh,

Here is what I observed in your latest PRD edits:

PATTERNS DETECTED
─────────────────
• You are making fast progress on Success Metrics (2 of 4 placeholders filled).
• JTBD section has been untouched for 3 cycles — this is the highest-risk gap given
  your 20-point checklist requires JTBD for both primary and secondary personas.
• No edits to Risks & Dependencies — this section is still entirely placeholder.

WHY THESE PATTERNS MATTER
──────────────────────────
Metrics-first editing suggests you are anchoring on measurability early — good signal.
But skipping JTBD means requirements written later may drift from actual user needs.
Risks left blank until the end often surface blocking dependencies too late for scoping.

SUGGESTED CHECKLIST ADDITIONS
───────────────────────────────
□ Add JTBD for Legal Ops Analyst persona
□ Define guardrail metric for extraction latency
□ Document top 3 risks with mitigation owners
□ Validate north-star metric target (40%) against baseline

[ APPROVE AND ADD TO CHECKLIST ]
```
...
prd-checklist.md
  ...
  prd.md  ──────────────────────────────────────────────────┐
     │                                                        │  (you keep editing)
     ▼ Prompt 2                                              │
dummy_prd.md (frozen baseline)                              │
     │                                                        │
     ▼ Prompt 3 + Prompt 5 (loop tick every 5 min)          │
  diff(prd.md, dummy_prd.md)  ◄────────────────────────────┘
     │
     ├──► prd-pattern-log.md  (append)
     │
     ├──► Email  (Prompt 4)
     │         │
     │         ▼  [Approve]
     │    prd-checklist.md  (updated with approved items)
     │
     └──► dummy_prd.md  (reset to current prd.md)
               │
               └──► next loop tick starts here
```
